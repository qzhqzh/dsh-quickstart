...
/**
 * Load the optional config file `~/.dsh-quickstart.json`. Returns `{}` when the
 * file is missing or unreadable so callers never crash on a bad config.
 * A `watch: true` entry opts the default `dsh-quickstart` command into watchdog
 * mode; without it (or with `watch: false`), the default stays plain launch.
 */
function loadConfig() {
  const configPath = path.join(os.homedir(), '.dsh-quickstart.json')
  try {
...
/**
 * Run dsh under a watchdog: restart it automatically whenever the process
 * exits (e.g. after a plugin install that requires a restart). The parent stays
 * alive to supervise — Ctrl-C / SIGTERM stops the loop. This is opt-in: the
 * default `dsh-quickstart` command stays a plain launch unless the user sets
 * `watch: true` in the config or passes `--watch`.
 */
function watch(opts = {}) {
  const {
    port = DEFAULT_PORT,
  ...
  const start = () => {
    ...
    child.on('exit', (code, signal) => {
      ...
      if (restarts >= maxRestarts) {
        log.error(`dsh-quickstart: dsh exited after ${maxRestarts} restarts — giving up`)
        process.exit(1)
      }
      restarts += 1
      log.warn(`dsh-quickstart: dsh exited (code=${code} signal=${signal}) — restarting in ${restartDelayMs}ms (${restarts}/${maxRestarts})`)
  ...
  // Open the browser once dsh first answers. If dsh is crash-looping we keep
  // polling rather than giving up after one timeout.
  const ensureOpened = async () => {
    while (!stopping && !opened) {
      try {
        await waitForServer(url, timeout)
        if (!opened) {
          opened = true
          log.info(`dsh-quickstart: dsh ready at ${url}`)
          if (open) openBrowser(url)
        }
        return
      } catch {
  ...
  const shutdown = () => { stopping = true; process.exit(0) }
